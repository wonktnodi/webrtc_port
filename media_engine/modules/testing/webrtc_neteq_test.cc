...
TEST_F(NetEqDecodingTest, TestAverageInterArrivalTimeNegative) {
  const int kNumFrames = 10;  // Needed for convergence.
  int frame_index = 0;
...
//    WebRtcNetEQ_RTPInfo rtp_info;
//    WebRtcNetEQ_NetworkStatistics net_stat;
//
...
//      kPayloadBytes, 0));
//    ASSERT_EQ(0, WebRtcNetEQ_GetNetworkStatistics(neteq_inst_->instance(), &net_stat));
//    /*printf("%d, %d, %d, %d, %d, %d, %d, %d\n",
//      net_stat.currentBufferSize, net_stat.preferredBufferSize,
//      net_stat.jitterPeaksFound, net_stat.currentPacketLossRate,
//      net_stat.currentDiscardRate, net_stat.currentExpandRate,
//      net_stat.currentPreemptiveRate, net_stat.currentAccelerateRate);*/
//  }
//  // Pull out all data.
//  for (int i = 0; i < num_frames; ++i) {
//    ASSERT_TRUE(kBlockSize16kHz == neteq_inst_->recOut(out_data_));
