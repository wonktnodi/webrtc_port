...
static WebRtc_UWord32 moduleFilter = kTraceModuleAll;
...
bool TraceImpl::TraceCheck(const TraceLevel level, const TraceModule module) const
{
    bool result = (level & levelFilter)? true:false;
    return (result && (module & moduleFilter)? true:false);
}
...
WebRtc_Word32 Trace::SetLevelFilter(const WebRtc_UWord32 filter,
  const WebRtc_UWord32 module)
{
    levelFilter = filter;
    moduleFilter = module;
    return 0;
...
WebRtc_Word32 Trace::LevelFilter(WebRtc_UWord32& filter, WebRtc_UWord32& module)
{
    filter = levelFilter;
    module = moduleFilter;
    return 0;
...

{
    ...
    if(trace)
    {
        if(trace->TraceCheck(level, module))
        {
