...
void TransmitMixer::RecordFileEnded(const WebRtc_Word32 id) {
  WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, -1),
...
WebRtc_Word32 TransmitMixer::Create(TransmitMixer*& mixer,
...
void TransmitMixer::Destroy(TransmitMixer*& mixer) {
  if (mixer) {
    delete mixer;
    mixer = NULL;
  }
...
WebRtc_Word32 TransmitMixer::SetEngineInformation(ProcessThread& processThread,
...
WebRtc_Word32 TransmitMixer::RegisterVoiceEngineObserver(VoiceEngineObserver& observer) {
  WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, -1),
...
WebRtc_Word32 TransmitMixer::DemuxAndMix() {
  WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, -1),
  ...
  while (channelPtr != NULL) {
    if (channelPtr->InputIsOnHold()) {
      ...
      // load temporary audio frame with current (mixed) microphone signal
...
WebRtc_Word32 TransmitMixer::EncodeAndSend() {
  WEBRTC_TRACE(kTraceStream, kTraceVoice, VoEId(_instanceId, -1),
...
WebRtc_Word32 TransmitMixer::StopSend() {
  WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, -1),
...
void TransmitMixer::SetMixWithMicStatus(bool mix) {
...
int TransmitMixer::SetMute(bool enable) {
  WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, -1),
...
void TransmitMixer::SetScale(float scale) {
	WEBRTC_TRACE(kTraceInfo, kTraceVoice, VoEId(_instanceId, -1),
		"TransmitMixer::SetScale(scale=%f)", scale);

	_scale = scale;
}

bool TransmitMixer::Mute() const {
  return _mute;
}
