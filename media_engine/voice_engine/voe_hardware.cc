...
VoEHardwareImpl::VoEHardwareImpl(SharedData* shared) : _shared(shared) {
...
int VoEHardwareImpl::SetRecordingDevice(int index,
  ...
  if (isRecording) {
	  WEBRTC_TRACE(kTraceInfo, kTraceVoice,  VoEId(_shared->instance_id(), -1), "Recording ok ...");
...
int VoEHardwareImpl::InputDevice(int &index) {
  WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
    "InputDevice");
  ...
  if (_shared->audio_device()->InputDevice(&index))
    return -1;
...
int VoEHardwareImpl::OutputDevice(int &index) {
  WEBRTC_TRACE(kTraceApiCall, kTraceVoice, VoEId(_shared->instance_id(), -1),
    "OutputDevice");
  ...
  if (_shared->audio_device()->OutputDevice(&index))
    return -1;
