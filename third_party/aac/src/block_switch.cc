/****************************************************************************
 ...
 $Id: block_switch.c,v 1.1.2.4 2004/02/16 20:02:18 ehr Exp $
 
****************************************************************************/
/*!
  \file
  \brief  Block Switching $Revision: 1.1.2.4 $
  \author M. Werner
*/
#include "psy_const.h"
                       ...
                       const int bitRate, const int nChannels)
{
  ...
  FLC_sub_start("InitBlockSwitching");
  ...
  PTR_INIT(1); /* blockSwitchingControl->iirStates[] */
  LOOP(1);
  for (i = 0; i < BLOCK_SWITCHING_IIR_LEN; i++) {
    MOVE(1);
    blockSwitchingControl->iirStates[i] = 0.0;
  }
  
  /* Clear Filtered Window Energies */
  PTR_INIT(2); /* blockSwitchingControl->windowNrgF[0][w]
                  blockSwitchingControl->windowNrgF[1][w]
               */
  LOOP(1);
  for (w = 0; w < BLOCK_SWITCH_WINDOWS; w++) {
    MOVE(2);
    blockSwitchingControl->windowNrgF[0][w] = 0.0;
    blockSwitchingControl->windowNrgF[1][w] = 0.0;
  }
  INDIRECT(1); MOVE(1);
  blockSwitchingControl->accWindowNrg = 0.0;
  
  PTR_INIT(2); /* blockSwitchingControl->windowNrg[0][w]
                  blockSwitchingControl->windowNrg[1][w]
               */
  LOOP(1);
  for (w = 0; w < BLOCK_SWITCH_WINDOWS; w++) {
    MOVE(2);
    blockSwitchingControl->windowNrg[0][w] = 0.0;
    blockSwitchingControl->windowNrg[1][w] = 0.0;
  }
...
static float SrchMaxWithIndex(const float in[], int *index, int n)
{
  ...
  FLC_sub_start("SrchMaxWithIndex");
                            ...
                            int windowLen)
{
    ...
    FLC_sub_start("CalcWindowEnergy");
                       ...
                       const int nChannels)
{
  ...
  FLC_sub_start("SyncBlockSwitching");
