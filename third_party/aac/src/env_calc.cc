/****************************************************************************
 ...
 $Id: env_calc.c,v 1.1.2.5 2004/02/17 15:36:16 jr Exp $

*******************************************************************************/
/*!
  \file
  \brief  Envelope calculation $Revision: 1.1.2.5 $
*/

#include <assert.h>
#include "env_calc.h"
#include "sbr_const.h"
#include "freq_sca_dec.h"
#include "env_extr.h"
                  ...
                  int noSubbands)
{
  ...
  FLC_sub_start("aliasingReduction");
...
*/
int
createSbrEnvelopeCalc (HANDLE_SBR_CALCULATE_ENVELOPE hs,
                       HANDLE_SBR_HEADER_DATA_DEC hHeaderData,
                       int chan)
{
  int err = 0;
  ...
  FLC_sub_start("createSbrEnvelopeCalc");
  ...
  PTR_INIT(2);
  hs->filtBuffer      = hHeaderData->sbr_GainSmooth[chan];
  hs->filtBufferNoise = hHeaderData->sbr_NoiseSmooth[chan];
...
{
  FLC_sub_start("resetSbrEnvelopeCalc");
                             ...
                             int     noSubbands)
{
  ...
  FLC_sub_start("updateFiltBuffer");
                              ...
                              )
{
  ...
  FLC_sub_start("calcNrgPerSubband");
                             ...
                             int    *ptrPhaseIndex)
{
  ...
  FLC_sub_start("adjustTimeSlotLP");
  ...
  *ptrReal++ = signalReal;
  ...
  if (noSubbands > 1) {
    ...
    case 1:
      ...
      if(tone_count <= 16 && k + lowSubband < 62)
#endif
      {
        BRANCH(1);
        if (freqInvFlag) {
                    ...
                    int limiterBands)
{
  ...
  FLC_sub_start("ResetLimiterBands");
