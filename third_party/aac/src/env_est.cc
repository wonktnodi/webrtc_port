/****************************************************************************
 ...
 $Id: env_est.c,v 1.1.2.14 2004/05/31 12:50:24 ehr Exp $

*******************************************************************************/
/*!
  \file
  \brief  Envelope estimation $Revision: 1.1.2.14 $
                                 ...
                                 )
{
  ...
  FLC_sub_start("sbrNoiseFloorLevelsQuantisation");
                 ...
                 )
{
  ...
  FLC_sub_start("coupleNoiseFloor");
...

{
  ...
  FLC_sub_start("calculateSbrEnvelope");
                    ...
                    HANDLE_COMMON_DATA hCmonData)
{
  ...
  FLC_sub_start("extractSbrEnvelope");
                          ...
                          )
{
  ...
  FLC_sub_start("CreateExtractSbrEnvelope");
  ...
  for (i = 0; i < YBufferLength; i++) {
    ...
    hSbrCut->YBuffer[i] = &hEnvEncoder->sbr_envYBuffer[chan*YBufferLength*64 + i*QMF_CHANNELS];

    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(QMF_CHANNELS);
    memset( hSbrCut->YBuffer[i],0,QMF_CHANNELS*sizeof(float));
  }
  ...
  for (i = 0; i < rBufferLength; i++) {
    INDIRECT(1); ADD(1); PTR_INIT(1);
    hSbrCut->rBuffer[i] = &hSbrCut->sbr_envRBuffer[chan*QMF_TIME_SLOTS*QMF_CHANNELS + i*QMF_CHANNELS];

    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(QMF_CHANNELS);
    memset( hSbrCut->rBuffer[i],0,QMF_CHANNELS*sizeof(float));

    INDIRECT(1); ADD(1); PTR_INIT(1);
    hSbrCut->iBuffer[i] = &hSbrCut->sbr_envIBuffer[chan*QMF_TIME_SLOTS*QMF_CHANNELS + i*QMF_CHANNELS];

    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(QMF_CHANNELS);
    memset( hSbrCut->iBuffer[i],0,QMF_CHANNELS*sizeof(float));
...
deleteExtractSbrEnvelope (HANDLE_SBR_EXTRACT_ENVELOPE hSbrCut)
{
  ...
  FLC_sub_start("deleteExtractSbrEnvelope");
