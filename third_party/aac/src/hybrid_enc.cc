/****************************************************************************
 ...
 $Id: hybrid.c,v 1.1.2.10 2004/02/17 10:21:22 jr Exp $

*******************************************************************************/
/*!
  \file
  \brief  Hybrid Filter Bank: 0.0 $
  \author Per Ekstrand
*/
#include <stdlib.h>
#include <string.h>
                                  ...
                                  int chOffset)
{
  ...
  FLC_sub_start("fourChannelFiltering");
                                   ...
                                   float **mHybridImag)
{
  ...
  FLC_sub_start("eightChannelFiltering");
...

{
  ...
  FLC_sub_start("HybridAnalysis_enc");
                  ...
                  HANDLE_HYBRID /*hHybrid*/ )      /*!< Handle to HYBRID struct. */
{
  ...
  FLC_sub_start("HybridSynthesis_enc");
  MOVE(1);
...
/**************************************************************************/
/*!
  \brief    CreateHybridFilterBank_enc
  \return   errorCode, noError if successful.
*/
/**************************************************************************/
int
CreateHybridFilterBank_enc ( HANDLE_HYBRID hs,         /*!< Handle to HYBRID struct. */
                         float **pPtr)
{
  int i;
  float *ptr = *pPtr;

  FLC_sub_start("CreateHybridFilterBank_enc");
  ...
  for (i = 0; i < NO_QMF_BANDS_IN_HYBRID; i++) {
    ...
    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(QMF_BUFFER_MOVE);
    memset(hs->mQmfBufferReal[i],0,QMF_BUFFER_MOVE*sizeof(float));

    ADD(1); STORE(1);
    hs->mQmfBufferImag[i] = ptr;ptr+= QMF_BUFFER_MOVE;

    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(QMF_BUFFER_MOVE);
    memset(hs->mQmfBufferImag[i],0,QMF_BUFFER_MOVE*sizeof(float));
  }
...
{
  FLC_sub_start("DeleteHybridFilterBank");
  FLC_sub_end();
  return;
}
