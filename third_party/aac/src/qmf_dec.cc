/****************************************************************************
 ...
 $Id: qmf_dec.c,v 1.1.2.6 2004/05/31 12:53:11 ehr Exp $

****************************************************************************/
/*
  \file
  \brief  Complex qmf analysis/synthesis, $Revision: 1.1.2.6 $
*/
#include <math.h>
#include <stdlib.h>
#include <stdio.h>
                        ...
                        )
{
  ...
  FLC_sub_start("sbrForwardModulationLP");
...
sbrForwardModulation (const float *timeIn,
                      float *rSubband,
                      float *iSubband,
                      HANDLE_SBR_QMF_FILTER_BANK_DEC anaQmf
                      )
{
  int i, offset;
  ...
  FLC_sub_start("sbrForwardModulation");
                          ...
                          )
{
  ...
  FLC_sub_start("cplxAnalysisQmfFiltering");
  ...
  for (i = 0; i < anaQmf->no_col; i++) {
    ...
    memcpy(anaQmf->FilterStatesAna+(QMF_FILTER_STATE_ANA_SIZE-NO_ANALYSIS_CHANNELS),ptr_timeIn,NO_ANALYSIS_CHANNELS*sizeof(float));
                                                                       ...
                                                                       FLC_sub_start("AnalysisPolyphaseFiltering");
...
inverseModulationLP (float *qmfReal,
                     float *qmfReal2,
                     HANDLE_SBR_QMF_FILTER_BANK_DEC synQmf
                     )
{
  int i, L, M;
  float timeOut[2*NO_ACTUAL_SYNTHESIS_CHANNELS];

  FLC_sub_start("inverseModulationLP");
                           ...
                           )
{
  ...
  FLC_sub_start("cplxSynthesisQmfFiltering");
  ...
  for (i = 0; i < synQmf->no_col; i++) {
    ...
    FLC_sub_start("SynthesisPolyphaseFiltering");
                           ...
                           int chan)
{
  ...
  FLC_sub_start("createCplxAnalysisQmfBank");


  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(sizeof(SBR_QMF_FILTER_BANK));
  memset(h_sbrQmf,0,sizeof(SBR_QMF_FILTER_BANK_DEC));
  ...
  PTR_INIT(1);
  h_sbrQmf->FilterStatesAna   = &(hHeaderData->sbr_CodecQmfStatesAnalysis[chan*QMF_FILTER_STATE_ANA_SIZE]);
                            ...
                            int bDownSample)
{
  ...
  FLC_sub_start("createCplxSynthesisQmfBank");
  ...
  PTR_INIT(1);
  h_sbrQmf->FilterStatesSyn = &h_sbrQmf->sbr_QmfStatesSynthesis[chan*qmfFilterStateSize];
