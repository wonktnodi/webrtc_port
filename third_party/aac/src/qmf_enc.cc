/****************************************************************************
 ...
 $Id: qmf_enc.c,v 1.1.2.15 2004/05/30 13:58:33 ehr Exp $

*******************************************************************************/
/*!
  \file
  \brief  QMF analysis $Revision: 1.1.2.15 $
...
*******************************************************************************/
static void
cosMod (float *subband, HANDLE_SBR_QMF_FILTER_BANK_ENC qmfBank)
{
  int i;
  float wim, wre;
...
*******************************************************************************/
static void
sinMod (float *subband, HANDLE_SBR_QMF_FILTER_BANK_ENC qmfBank)
{
  int i;
  float wre, wim;
  float re1, im1, re2, im2;
                   ...
                   HANDLE_SBR_QMF_FILTER_BANK_ENC qmfBank)
{
  ...
  FLC_sub_start("inverseModulation");
                   ...
                   )
{
  ...
  FLC_sub_start("forwardModulation");
                      ...
                      )
{
  ...
  FLC_sub_start("sbrAnalysisFiltering");
                         ...
                         )
{
  ...
  FLC_sub_start("getEnergyFromCplxQmfData");
...
createQmfBank (int chan,
               HANDLE_SBR_QMF_FILTER_BANK_ENC   h_sbrQmf
               ...
               )
{
  ...
  FLC_sub_start("createQmfBank");

  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(sizeof(SBR_QMF_FILTER_BANK));
  memset (h_sbrQmf, 0, sizeof (SBR_QMF_FILTER_BANK_ENC));
  ...
  h_sbrQmf->qmf_states_buffer = h_sbrQmf->sbr_QmfStatesAnalysis + chan * 640;

  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(640);
  memset(h_sbrQmf->qmf_states_buffer, 0, 640 * sizeof(float));
...
void
deleteQmfBank (
               HANDLE_SBR_QMF_FILTER_BANK_ENC  /*h_sbrQmf*/ /*!<pointer to QMF filter bank instance */
               )
{

  FLC_sub_start("deleteQmfBank");
  /*
    nothing to do
  */
  FLC_sub_end();
                       ...
                       HANDLE_SBR_QMF_FILTER_BANK_ENC qmfBank)
{
  ...
  FLC_sub_start("SynthesisQmfFiltering");
...
*******************************************************************************/
int
CreateSynthesisQmfBank (HANDLE_SBR_QMF_FILTER_BANK_ENC hs)
{
  float *ptr;

  FLC_sub_start("CreateSynthesisQmfBank");

  PTR_INIT(1);
  ptr = &hs->PsBuf5[0];

  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(sizeof(SBR_QMF_FILTER_BANK));
  memset (hs, 0, sizeof (SBR_QMF_FILTER_BANK_ENC));
...
*******************************************************************************/
void
DeleteSynthesisQmfBank (HANDLE_SBR_QMF_FILTER_BANK_ENC * /*h_sbrQmf*/)
{
  FLC_sub_start("DeleteSynthesisQmfBank");
  FLC_sub_end();
