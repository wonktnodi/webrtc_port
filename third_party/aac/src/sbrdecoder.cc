/****************************************************************************
 ...
 $Id: sbrdecoder.c,v 1.1.2.13 2004/05/31 12:53:11 ehr Exp $

*******************************************************************************/
/*!
  \file
  \brief  SBR decoder frontend $Revision: 1.1.2.13 $
*/
          ...
          )
{
 ...
 PTR_INIT(1); /* SbrChannel[] */
    ...
    if (err) {
      ...
      if (dualMono)  {
        ...
        if(hHeaderData->channelMode == PS_STEREO ) {
          PTR_INIT(1); FUNC(1);
          DecodePs(&self->ParametricStereoDec);
        }
#endif /* #ifndef MONO_ONLY */
  ...
  if (!bBitstreamDownMix && (stereo || dualMono)) {
    ...
    FUNC(2); LOOP(1); PTR_INIT(2); MOVE(1); STORE(codecFrameSize);
    memcpy( self->InterimResult, pWorkBuffer1+codecFrameSize, codecFrameSize*sizeof(float) );
  ...
  PTR_INIT(3); ADD(1); FUNC(10);
    ...
    if(bDownSample){
      ...
      FUNC(2); LOOP(1); PTR_INIT(2); MOVE(1); STORE(codecFrameSize);
      memcpy( &(timeData[codecFrameSize]), self->InterimResult, codecFrameSize*sizeof(float) );
    }
  }
  else {
...
map2channelSbrData2one(HANDLE_SBR_FRAME_DATA hFrameDataLeft,
  ...
  FLC_sub_start("map2channelSbrData2one");
