/****************************************************************************
 ...
 $Id: sf_estim.c,v 1.1.2.5 2004/02/16 21:05:58 ehr Exp $
 
****************************************************************************/
/*!
  \file 
  \brief  Scalefactor estimation $Revision: 1.1.2.5 $
  \author M. Werner
*/
#include <memory.h>
#include <limits.h>
                      ...
                      PSY_OUT_CHANNEL *psyOutChan)
{
  ...
  FLC_sub_start("CalcFormFactorChannel");
  ...
  for (sfbOffs=0; sfbOffs<psyOutChan->sfbCnt; sfbOffs+=psyOutChan->sfbPerGroup){
    ...
    for (sfb=0; sfb<psyOutChan->maxSfbPerGroup; sfb++) {
      ...
      if (psyOutChan->sfbEnergy[i] > psyOutChan->sfbThreshold[i]) {
        ...
        DIV(1); TRANS(1);
        avgFormFactor = (float) pow(psyOutChan->sfbEnergy[i]/(float)sfbWidth, 0.25f);
...
static int countSingleScfBits(int scf, int scfLeft, int scfRight)
{
  ...
  FLC_sub_start("countSingleScfBits");
...
static float calcSingleSpecPe(int scf, float sfbConstPePart, float nLines)
{
  ...
  FLC_sub_start("calcSingleSpecPe");
                            ...
                            int sfbCnt, int startSfb, int stopSfb)
{
  ...
  FLC_sub_start("countScfBitsDiff");
                                ...
                                int restartOnSuccess)
{
  ...
  FLC_sub_start("assimilateSingleScf");
                                  ...
                                  float *sfbNRelevantLines)
{
  ...
  FLC_sub_start("assimilateMultipleScf");
                            ...
                            short *quantSpec)
{
  ...
  FLC_sub_start("EstimateScaleFactorsChannel");
  ...
  for(i=0; i<psyOutChan->sfbCnt; i++) {
    ...
    if( (maxSpec > 0.0) && (energy > thresh) ){
        ...
        ADD(3); FUNC(3);
        calcExpSpec(psyOutChan->expSpec+psyOutChan->sfbOffsets[i],
        ...
        scfInt = improveScf(psyOutChan->mdctSpectrum+psyOutChan->sfbOffsets[i],
                            psyOutChan->expSpec+psyOutChan->sfbOffsets[i],
                            quantSpec+psyOutChan->sfbOffsets[i],
                            psyOutChan->quantSpecTmp+psyOutChan->sfbOffsets[i],
                            psyOutChan->sfbOffsets[i+1]-psyOutChan->sfbOffsets[i],
  ...
  
  {
    ...
    FUNC(12);
    assimilateSingleScf(psyOutChan, psyOutChan->expSpec, quantSpec, psyOutChan->quantSpecTmp, scf, 
                         minSfMaxQuant, sfbDist, sfbConstPePart,
                         sfbFormFactor, sfbNRelevantLines, minScfCalculated, 1);

    FUNC(10);
    assimilateMultipleScf(psyOutChan, psyOutChan->expSpec, quantSpec, psyOutChan->quantSpecTmp, scf, 
                          minSfMaxQuant, sfbDist, sfbConstPePart,
                          sfbFormFactor, sfbNRelevantLines);
  }
                     ...
                     const    int nChannels)
{
   ...
   FLC_sub_start("EstimateScaleFactors");
