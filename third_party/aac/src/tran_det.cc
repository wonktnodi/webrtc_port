/****************************************************************************
 ...
 $Id: tran_det.c,v 1.1.2.7 2004/02/17 18:37:57 fh Exp $

*******************************************************************************/
/*!
  \file
  \brief  Transient detector $Revision: 1.1.2.7 $
                                ...
                                int slots)
{
  ...
  FLC_sub_start("addLowbandEnergies");
                                 ...
                                 int timeStep)
{
  ...
  FLC_sub_start("addHighbandEnergies");
                     ...
                     float  *thresholds)
{
  ...
  FLC_sub_start("calculateThresholds");
...

{
  ...
  FLC_sub_start("extractTransientCandidates");
    ...
    MULT(1); ADD(2); LOGIC(1);
      ...
      if (cond) {
        ...
        ADD(1); DIV(1); MISC(1); STORE(1);
	      tran_vector[0] =
	        (int) floor ( (float)(i - qmfStartSample) / (float)timeStep );
                            ...
                            )
{
  ...
  FLC_sub_start("CreateSbrTransientDetector");
  ...
  h_sbrTransientDetector->thresholds = &h_sbrTransientDetector->sbr_thresholds[chan*QMF_CHANNELS];
  ...
  h_sbrTransientDetector->transients =  &h_sbrTransientDetector->sbr_transients[chan*h_sbrTransientDetector->buffer_length];
...
{
  FLC_sub_start("deleteSbrTransientDetector");
