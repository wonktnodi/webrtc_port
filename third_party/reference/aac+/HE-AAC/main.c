...
int main(int argc, char *argv[])
{
  ...
  fprintf(stderr,"\n");
  fprintf(stderr,"*************************************************************\n");
  fprintf(stderr,"* Coding Technologies' enhanced aacPlus 3GPP Reference Encoder\n");
  fprintf(stderr,"* Build %s, %s\n", __DATE__, __TIME__);
  fprintf(stderr,"* Use of this product is permitted exclusively during the\n");
  fprintf(stderr,"* course of 3GPP codec evaluation.\n");
  fprintf(stderr,"*  (c) 2000 - 2004 Coding Technologies\n");
  fprintf(stderr,"*  (c) 1999 Fraunhofer Gesellschaft IIS-A\n");
  fprintf(stderr,"*\n");
  fprintf(stderr,"*************************************************************\n\n");
  ...
  */
  ...
  if (bDoUpsample) {
    ...
    PTR_INIT(1); FUNC(3);
    InitIIR21_Resampler(&(IIR21_reSampler[0]),transitionFac,2);

#if (MAX_CHANNELS==2)
    PTR_INIT(1); FUNC(3);
    InitIIR21_Resampler(&(IIR21_reSampler[1]),transitionFac,2);
#endif
  ...
  */
  ...
  while (1)
  {
    ...
    if (bDoIIR32Resample) {
      ...
      for (r=0; r<nRuns; r++) {
        ...
        nSamplesProcessed += IIR32Resample( resamplerScratch,
                                            &inputBuffer[writeOffset + r*nDSOutBlockSize*inputInfo.nChannels],
                                            nSamplesPerChannel,
                                            nDSOutBlockSize,
                                            inputInfo.nChannels);
      }
    ...
    if (hEnvEnc && useParametricStereo) {
      ...
      if(hEnvEnc)
      {
        FUNC(2); LOOP(1); PTR_INIT(2); MOVE(1); STORE(CORE_INPUT_OFFSET_PS);
