/****************************************************************************
 ...
 $Id: aacenc.c,v 1.1.2.21 2004/05/30 14:15:19 ehr Exp $

****************************************************************************/
/*!
  \file
  \brief  fast aac coder interface library functions $Revision: 1.1.2.21 $
...
{
  FLC_sub_start("AacInitDefaultConfig");
             ...
             )
{
  ...
  FLC_sub_start("AacEncOpen");
  
  MOVE(2); /* counting previous operations */
  
  PTR_INIT(1);
  hAacEnc = new AAC_ENCODER;
  
  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(sizeof(struct AAC_ENCODER));
  memset(hAacEnc,0,sizeof(struct AAC_ENCODER));
