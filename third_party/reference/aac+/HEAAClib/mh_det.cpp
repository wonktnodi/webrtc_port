/****************************************************************************
 ...
 $Id: mh_det.c,v 1.1.2.10 2004/04/01 13:14:17 ehr Exp $

*******************************************************************************/
/*!
  \file
  \brief  Missing harmonics detection $Revision: 1.1.2.10 $
                                     ...
                                     int nSfb)
{
  ...
  FLC_sub_start("calculateFlatnessMeasure");
                                   ...
                                   int noQmfBands)
{
  ...
  FLC_sub_start("calculateDetectorInput");
                                       ...
                                       HANDLE_SBR_MISSING_HARMONICS_DETECTOR h_sbrMissingHarmonicsDetector)
{
  ...
  FLC_sub_start("isDetectionOfNewToneAllowed");
                             ...
                             int stop)
{
  ...
  FLC_sub_start("transientCleanUp");
                                    ...
                                    unsigned char* pAddHarmonicsScaleFactorBands)
{
  ...
  FLC_sub_start("detectionWithPrediction");
                                ...
                                int newDetectionAllowed)
{
  ...
  FLC_sub_start("calculateCompVector");
                               ...
                               char* envelopeCompensation)
{
  ...
  FLC_sub_start("SbrMissingHarmonicsDetectorQmf");
                                   ...
                                   int noEstPerFrame)
{
  ...
  FLC_sub_start("CreateSbrMissingHarmonicsDetector");
  ...
  ptr = &hSbrMHDet->sbr_toncorrBuff[chan*5*NO_OF_ESTIMATES*MAX_FREQ_COEFFS];
  ...
  for(i=0; i < totNoEst; i++) {
    ...
    hs->detectionVectors[i] = &(hs->sbr_detectionVectors[chan*NO_OF_ESTIMATES*MAX_FREQ_COEFFS + i*MAX_FREQ_COEFFS]);

    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(MAX_FREQ_COEFFS);
    memset(hs->detectionVectors[i],0,sizeof(unsigned char)*MAX_FREQ_COEFFS);

    MULT(2); ADD(1); PTR_INIT(1); ADD(1);
    hs->guideVectors[i].guideVectorDetected = &(hs->sbr_guideVectorDetected[chan*NO_OF_ESTIMATES*MAX_FREQ_COEFFS + i*MAX_FREQ_COEFFS]);
  ...
  hs->prevEnvelopeCompensation = &(hs->sbr_prevEnvelopeCompensation[chan*MAX_FREQ_COEFFS]);

  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(MAX_FREQ_COEFFS);
  memset( hs->prevEnvelopeCompensation,0, sizeof(char)*MAX_FREQ_COEFFS);

  INDIRECT(2); MULT(1); PTR_INIT(1);
  hs->guideScfb = &(hs->sbr_guideScfb[chan*MAX_FREQ_COEFFS]);
  ...
  assert (ptr-&hSbrMHDet->sbr_toncorrBuff[0] <= 5*MAX_CHANNELS*NO_OF_ESTIMATES*MAX_FREQ_COEFFS);
