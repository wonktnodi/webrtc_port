/****************************************************************************
 ...
 $Id: ps_enc.c,v 1.1.2.18 2004/04/01 13:14:17 ehr Exp $

*******************************************************************************/
/*!
  \file
  \brief  parametric stereo encoding $Revision: 1.1.2.18 $
			...
			float* sbr_envYBuffer)
{
  ...
  FLC_sub_start("CreatePsEnc");

  PTR_INIT(4);
  ptr1 = &sbr_envYBuffer[QMF_TIME_SLOTS * QMF_CHANNELS];
  ptr2 = &h_ps_e->mdctDelayBuffer_2[BLOCK_SWITCHING_OFFSET];
  ptr3 = h_ps_e->PsBuf3;
  ...
  for (i=0 ; i < NO_BINS ; i++) {
    ...
    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(SYSTEMLOOKAHEAD + 1);
    memset(h_ps_e->aaaICCDataBuffer[i],0,(SYSTEMLOOKAHEAD + 1)*sizeof(float));
    ...
    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(SYSTEMLOOKAHEAD + 1);
    memset(h_ps_e->aaaIIDDataBuffer[i],0,(SYSTEMLOOKAHEAD + 1)*sizeof(float));
  }
  ...
  FUNC(2);
  err = CreateHybridFilterBank_enc ( h_ps_e->hHybridLeft,
                                 &ptr4);
  ...
  FUNC(2);
  err = CreateHybridFilterBank_enc ( h_ps_e->hHybridRight,
                                 &ptr4);
  ...
  for (i=0 ; i < NO_SUBSAMPLES ; i++) {
    ...
    FUNC(2); LOOP(1); MOVE(1); STORE(NO_HYBRID_BANDS);
    memset(h_ps_e->mHybridRealLeft[i],0,NO_HYBRID_BANDS*sizeof(float));
    ...
    FUNC(2); LOOP(1); MOVE(1); STORE(NO_HYBRID_BANDS);
    memset(h_ps_e->mHybridImagLeft[i],0,NO_HYBRID_BANDS*sizeof(float));
    ...
    FUNC(2); LOOP(1); MOVE(1); STORE(NO_HYBRID_BANDS);
    memset(h_ps_e->mHybridRealRight[i],0,NO_HYBRID_BANDS*sizeof(float));
    ...
    FUNC(2); LOOP(1); MOVE(1); STORE(NO_HYBRID_BANDS);
    memset(h_ps_e->mHybridImagRight[i],0,NO_HYBRID_BANDS*sizeof(float));
  }
  ...
  for (i=0 ; i < HYBRID_FILTER_DELAY ; i++) {
    ...
    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(NO_QMF_BANDS);
    memset(h_ps_e->tempQmfLeftReal[i],0,NO_QMF_BANDS*sizeof(float));
    ...
    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(NO_QMF_BANDS);
    memset(h_ps_e->tempQmfLeftImag[i],0,NO_QMF_BANDS*sizeof(float));
    ...
    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(NO_QMF_BANDS);
    memset(h_ps_e->histQmfLeftReal[i],0,NO_QMF_BANDS*sizeof(float));
    ...
    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(NO_QMF_BANDS);
    memset(h_ps_e->histQmfLeftImag[i],0,NO_QMF_BANDS*sizeof(float));
    ...
    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(NO_QMF_BANDS);
    memset(h_ps_e->histQmfRightReal[i],0,NO_QMF_BANDS*sizeof(float));
    ...
    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(NO_QMF_BANDS);
    memset(h_ps_e->histQmfRightImag[i],0,NO_QMF_BANDS*sizeof(float));
  }

  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(sizeof(h_ps_e->powerLeft));
  memset( h_ps_e->powerLeft    , 0, sizeof( h_ps_e->powerLeft     ) );

  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(sizeof(h_ps_e->powerRight));
  memset( h_ps_e->powerRight   , 0, sizeof( h_ps_e->powerRight    ) );

  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(sizeof(h_ps_e->powerCorrReal));
  memset( h_ps_e->powerCorrReal, 0, sizeof( h_ps_e->powerCorrReal ) );

  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(sizeof(h_ps_e->powerCorrImag));
  memset( h_ps_e->powerCorrImag, 0, sizeof( h_ps_e->powerCorrImag ) );
  ...
  PTR_INIT(2); /* h_ps_e->aaaIIDDataBuffer[][]
                  h_ps_e->aaaICCDataBuffer[][] */
  LOOP(1);
  for (i=0; i<h_ps_e->iidIccBins; i++)
  {
    MOVE(2);
    h_ps_e->aaaIIDDataBuffer[i][0] = 0.0f;
              ...
              float **rBufferRight)
{
  ...
  FLC_sub_start("EncodePsFrame");

  INDIRECT(3); FUNC(5);
  HybridAnalysis_enc ( (const float**) rBufferLeft,
  ...
  INDIRECT(3); FUNC(5);
  HybridAnalysis_enc ( (const float**) rBufferRight,
...
#ifdef NON_BE_BUGFIX
    /* HP20040217 maybe one should set groupBordersMix[group + 1] to stopfreq */
    ADD(1); BRANCH(1);
    if (maxSubband > MAX_FREQ_COEFFS) {
      MOVE(1);
      maxSubband = MAX_FREQ_COEFFS;
    }
#endif
               ...
               float **qmfRightImag )
{
  ...
  PTR_INIT(2); /* bins2groupMap_enc[group]
  ...
  for ( group = 0; group < NO_IPD_GROUPS; group++ ) {
    ...
    LOGIC(1);
    bin = ( ~NEGATE_IPD_MASK ) & bins2groupMap_enc[group];
...
#ifdef NON_BE_BUGFIX
    /* HP20040217 maybe one should set groupBordersMix[group + 1] to stopfreq, but SBR enc also looks at QMF bands above stopfreq :-( ... */
    ADD(1); BRANCH(1);
    if (maxSubband > MAX_FREQ_COEFFS) {
      MOVE(1);
      maxSubband = MAX_FREQ_COEFFS;
  ...
  INDIRECT(3); FUNC(5);
  HybridSynthesis_enc( ( const float** )pms->mHybridRealLeft,
