/****************************************************************************
 ...
 $Id: psy_main.c,v 1.1.2.19 2004/05/28 11:36:18 snd Exp $
 
****************************************************************************/
/*!
  \file 
  \brief  Psychoaccoustic main module $Revision: 1.1.2.19 $
  \author M. Werner
*/
#include <stdlib.h>
#include <string.h>
...
const static int blockType2windowShape[] = {KBD_WINDOW,SINE_WINDOW,SINE_WINDOW,KBD_WINDOW};
...
int PsyNew(PSY_KERNEL  *hPsy, int nChan)
{
    ...
    FLC_sub_start("PsyNew");
   
    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(sizeof(PSY_KERNEL));
    memset(hPsy,0,sizeof(PSY_KERNEL));  
    ...
    for (i=0; i<nChan; i++){
      ...
      */
      PTR_INIT(1);
      hPsy->psyData[i].mdctDelayBuffer = &hPsy->mdctDelayBuffer_2[i*BLOCK_SWITCHING_OFFSET];

      FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(BLOCK_SWITCHING_OFFSET);
      memset(hPsy->psyData[i].mdctDelayBuffer,0,BLOCK_SWITCHING_OFFSET*sizeof(float));
      ...
      */
      ...
      PTR_INIT(1);
      hPsy->psyData[i].mdctSpectrum = &hPsy->sbr_envRBuffer[i*FRAME_LEN_LONG];

      FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(FRAME_LEN_LONG);
      memset(hPsy->psyData[i].mdctSpectrum,0,FRAME_LEN_LONG*sizeof(float));

      
    }
  
    INDIRECT(1); PTR_INIT(1);
    hPsy->pScratchTns = hPsy->sbr_envIBuffer;

    FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(FRAME_LEN_LONG);
    memset(hPsy->pScratchTns,0,FRAME_LEN_LONG*sizeof(float));
                            ...
                            const int ch)
{
    ...
    FLC_sub_start("advancePsychLong");
...
static int advancePsychLongMS (PSY_DATA psyData[MAX_CHANNELS],
                               PSY_CONFIGURATION_LONG *psyConfLong)
{
    FLC_sub_start("advancePsychLongMS");
                             ...
                             const int ch)
{
    ...
    FLC_sub_start("advancePsychShort");
                                ...
                                PSY_CONFIGURATION_SHORT *psyConfShort)
{
    ...
    FLC_sub_start("advancePsychShortMS");
