/****************************************************************************
 ...
 $Id: sbr_main.c,v 1.1.2.33 2004/05/31 12:50:25 ehr Exp $

****************************************************************************/
/*!
  \file
  \brief  SBR encoder top level processing $Revision: 1.1.2.33 $
                       ...
                       )
{
  ...
  FLC_sub_start("getSbrTuningTableIndex");
                   ...
                   )
{
  ...
  FLC_sub_start("IsSbrSettingAvail");
                   ...
                   )
{
  ...
  FLC_sub_start("AdjustSbrSettings");
...
{
  FLC_sub_start("InitializeSbrDefaults");
...
deleteEnvChannel (HANDLE_ENV_CHANNEL hEnvCut)
{
  ...
  FLC_sub_start("deleteEnvChannel");
...
EnvClose (HANDLE_SBR_ENCODER hEnvEnc)
{
  ...
  if (hEnvEnc != NULL) {
    ...
    {
      PTR_INIT(1); FUNC(1);
      DeleteSynthesisQmfBank ((HANDLE_SBR_QMF_FILTER_BANK_ENC*)&hEnvEnc->hSynthesisQmfBank);
    }
...
static int updateFreqBandTable(HANDLE_SBR_CONFIG_DATA  sbrConfigData,
                               HANDLE_SBR_HEADER_DATA_ENC  sbrHeaderData,
                               int noQmfChannels)
{
  ...
  FLC_sub_start("updateFreqBandTable");
				  ...
				  )
{
  ...
  FLC_sub_start("createEnvChannel");

  MOVE(2); /* counting previous operations */

  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(sizeof(struct ENV_CHANNEL));
  memset(hEnv,0,sizeof (struct ENV_CHANNEL));
         ...
         int      *coreBandWith)
{
  ...
  FLC_sub_start("EnvOpen");

  MOVE(1);
  *hEnvEncoder=0;

  PTR_INIT(1);
  hEnvEnc = new SBR_ENCODER;

  FUNC(2); LOOP(1); PTR_INIT(1); MOVE(1); STORE(sizeof(struct SBR_ENCODER));
  memset(hEnvEnc,0, sizeof (struct SBR_ENCODER));
  ...
  {
    PTR_INIT(1);
    hEnvEnc->hEnvChannel[ch] = new ENV_CHANNEL;
  }
  ...
  hEnvEnc->sbrConfigData.freqBandTable[LO] =  hEnvEnc->sbrConfigData.sbr_freqBandTableLO;
  ...
  INDIRECT(1); PTR_INIT(1);
  hEnvEnc->sbrConfigData.freqBandTable[HI] =  hEnvEnc->sbrConfigData.sbr_freqBandTableHI;
  ...
  INDIRECT(1); PTR_INIT(1);
  hEnvEnc->sbrConfigData.v_k_master =  hEnvEnc->sbrConfigData.sbr_v_k_master;
  ...
  if (params->usePs)
  {
    INDIRECT(3); PTR_INIT(2); FUNC(5); BRANCH(1);
    if(createEnvChannel(1,
                        &hEnvEnc->sbrConfigData,
                        &hEnvEnc->sbrHeaderData,
                        hEnvEnc->hEnvChannel[1],
                        params,
						hEnvEnc)){
      FUNC(1);
      EnvClose (hEnvEnc);
    ...
    INDIRECT(1); MOVE(1);
    hEnvEnc->hSynthesisQmfBank = &hEnvEnc->synthesisQmfBank;
    ...
    INDIRECT(1); MOVE(1);
    hEnvEnc->hPsEnc = &hEnvEnc->PsEnc;
	memset(hEnvEnc->hPsEnc, 0 ,sizeof(PS_ENC));

    INDIRECT(2); PTR_INIT(1); FUNC(2); BRANCH(1);
    if(CreatePsEnc (hEnvEnc->hPsEnc, params->psMode, hEnvEnc->sbr_envYBuffer)){
