/****************************************************************************
 ...
 $Id: block.c,v 1.1.2.6 2004/04/23 07:16:55 hfm Exp $

****************************************************************************/
/*!
  \file
  \brief  long/short decoding  $Revision: 1.1.2.6 $
  \author zerok
*/
...
void CPns_Apply(AACDECODER self,CAacDecoderChannelInfo *pAacDecoderChannelInfo[], int channel);

float CBlock_Quantize(int value,
                      int scfMod,
                      int scale)
{
  FLC_sub_start("CBlock_Quantize");
  ...
  if (value != 0)
  {
    ...
    if (value >= 0.0) {
      TRANS(1); MULT(1);
      FLC_sub_end();
      return (float) (pow ((double)value, 4.0 / 3.0) * scalefactor) ;
    }
    else {
      TRANS(1); MULT(3);
      FLC_sub_end();
                             ...
                             const unsigned short (*CodeBook) [HuffmanEntries]) /*!< pointer to sideinfo of the current codebook */
{
  ...
  FLC_sub_start("CBlock_DecodeHuffmanWord");
                     ...
                     const int q)        /*!< quantized coefficient */
{
  ...
  FLC_sub_start("CBlock_GetEscape");
                       ...
                       const CodeBookDescription *hcb) /*!< pointer to sideinfo of the current codebook */
{
  ...
  FLC_sub_start("CBlock_UnpackIndex");
                     ...
                     unsigned char global_gain)                      /*!< global gain */
{
  ...
  FLC_sub_start("CShortBlock_Read");
...
void ApplyTools(AACDECODER self,CAacDecoderChannelInfo *pAacDecoderChannelInfo[],
                int channel)
{
  FLC_sub_start("ApplyTools");

  FUNC(2);
  CPns_Apply(self,pAacDecoderChannelInfo, channel);
