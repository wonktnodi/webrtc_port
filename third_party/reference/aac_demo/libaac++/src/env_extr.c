/****************************************************************************
 ...
 $Id: env_extr.c,v 1.1.2.12 2004/05/31 12:53:11 ehr Exp $

*******************************************************************************/
/*
  \file
  \brief  Envelope extraction $Revision: 1.1.2.12 $
*/
#include <string.h>
#include <assert.h>
...
static int sbrGetEnvelope (HANDLE_SBR_HEADER_DATA_DEC hHeaderData,
                           HANDLE_SBR_FRAME_DATA h_frame_data,
                           HANDLE_BIT_BUFFER hBitBuf);
static void sbrGetDirectionControlData (HANDLE_SBR_FRAME_DATA hFrameData,
                                        HANDLE_BIT_BUFFER hBitBuf);
static void sbrGetNoiseFloorData (HANDLE_SBR_HEADER_DATA_DEC hHeaderData,
                                  HANDLE_SBR_FRAME_DATA h_frame_data,
                                  HANDLE_BIT_BUFFER hBitBuf);
...
*/
void
initHeaderData (SBR_HEADER_DATA_DEC *hHeaderData,
                int sampleRate,
                int samplesPerFrame)
{
  HANDLE_FREQ_BAND_DATA hFreq = &hHeaderData->FreqBandData;

  FLC_sub_start("initHeaderData");

  PTR_INIT(1); MOVE(1); /* counting previous operation */

  FUNC(2); LOOP(1); PTR_INIT(2); MOVE(1); STORE(sizeof(SBR_HEADER_DATA_DEC));
  //memcpy(hHeaderData, &sbr_defaultHeader, sizeof(SBR_HEADER_DATA_DEC));
  hHeaderData->reset();
  ...
  INDIRECT(1); MOVE(1);
  //hHeaderData->FreqBandData = hFreq;
                      ...
                      int timeSlots)
{
  ...
  FLC_sub_start("initSbrPrevFrameData");
...
{
  SBR_HEADER_DATA_DEC lastHeader;
  int dummy;
  int headerExtra1, headerExtra2;

  FLC_sub_start("sbrGetHeaderData");


  FUNC(2); LOOP(1); PTR_INIT(2); MOVE(1); STORE(sizeof(SBR_HEADER_DATA_DEC));
  memcpy (&lastHeader, h_sbr_header, sizeof(SBR_HEADER_DATA_DEC));
...
*/
static int
sbrGetSyntheticCodedData(HANDLE_SBR_HEADER_DATA_DEC hHeaderData,
                         ...
                         HANDLE_BIT_BUFFER     hBitBuf)
{
  ...
  FLC_sub_start("sbrGetSyntheticCodedData");
...
*/
static void extractExtendedData(HANDLE_BIT_BUFFER     hBitBuf,
                                HANDLE_SBR_HEADER_DATA_DEC hHeaderData,
                                HANDLE_PS_DEC       hPs
                                )
{
  ...
  FLC_sub_start("extractExtendedData");
...
*/
int
sbrGetSingleChannelElement (HANDLE_SBR_HEADER_DATA_DEC hHeaderData,
                            HANDLE_SBR_FRAME_DATA  hFrameData,
                            HANDLE_PS_DEC          hPs,
                            HANDLE_BIT_BUFFER      hBitBuf)
{
  ...
  FLC_sub_start("sbrGetSingleChannelElement");
  ...
  FUNC(3);
  extractExtendedData(hBitBuf, hHeaderData, hPs);
...
*/
int
sbrGetChannelPairElement (HANDLE_SBR_HEADER_DATA_DEC hHeaderData,
                          ...
                          HANDLE_BIT_BUFFER hBitBuf)
{
  ...
  FLC_sub_start("sbrGetChannelPairElement");
  ...
  FUNC(3);
  extractExtendedData(hBitBuf, hHeaderData, NULL);
...
*/
void
sbrGetNoiseFloorData (HANDLE_SBR_HEADER_DATA_DEC hHeaderData,
                      HANDLE_SBR_FRAME_DATA h_frame_data,
                      HANDLE_BIT_BUFFER hBitBuf)
{
  int i,j;
  ...
  FLC_sub_start("sbrGetNoiseFloorData");
...
*/
int
sbrGetEnvelope (HANDLE_SBR_HEADER_DATA_DEC hHeaderData,
                HANDLE_SBR_FRAME_DATA h_frame_data,
                HANDLE_BIT_BUFFER hBitBuf)
{
  int i, j;
                  ...
                  HANDLE_SBR_FRAME_DATA h_frame_data)
{
  ...
  FLC_sub_start("extractFrameInfo");
