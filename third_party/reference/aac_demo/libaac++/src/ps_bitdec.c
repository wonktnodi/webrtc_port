/****************************************************************************
 ...
 $Id: ps_bitdec.c,v 1.1.2.11.2.2 2004/08/04 10:01:57 ehr Exp $

*******************************************************************************/
/*
  \file
  \brief  Parametric Stereo bitstream processing and decoding $Revision: 1.1.2.11.2.2 $
...
#else
static const int aNoIidBins[2] = {NO_LOW_RES_IID_BINS, NO_IID_BINS};
static const int aNoIccBins[2] = {NO_LOW_RES_ICC_BINS, NO_ICC_BINS};
#endif
                 ...
                 int maxIdx)
{
  ...
  FLC_sub_start( "deltaDecodeArray" );
...
DecodePs(struct PS_DEC *h_ps_dec)
{
...
#else
                     aNoIidBins[h_ps_dec->freqResIid?1:0],
#endif
...
#else
                                                                               INDIRECT( 4 ); BRANCH( 2 ); MOVE( 2 ); FUNC( 8 );
#endif
...
#else
                     aNoIccBins[h_ps_dec->freqResIcc?1:0],
#endif
  ...
  if (h_ps_dec->bFrameClass == 0) {
    ...
    if (h_ps_dec->aEnvStartStop[h_ps_dec->noEnv] <
      ...
      for (gr = 0; gr < NO_BINS; gr++) {
#endif
        h_ps_dec->aaIidIndex[h_ps_dec->noEnv][gr] =
          h_ps_dec->aaIidIndex[h_ps_dec->noEnv-1][gr];                        MOVE( 1 );
      }
                                                                                PTR_INIT( 2 ); LOOP( 1 );
#ifdef NON_BE_BUGFIX
      ...
      for (gr = 0; gr < NO_BINS; gr++) {
#endif
        h_ps_dec->aaIccIndex[h_ps_dec->noEnv][gr] =
          h_ps_dec->aaIccIndex[h_ps_dec->noEnv-1][gr];                        MOVE( 1 );
      }
    }
...
#else
  if ((h_ps_dec->freqResIid > 1) || (h_ps_dec->freqResIcc > 1)) {
#endif
...
#else
                                                                                PTR_INIT( 1 ); BRANCH( 1 ); INDIRECT( 2 ); LOOP( 1 );
      for (gr = 0; gr < aNoIidBins[h_ps_dec->freqResIid?1:0]; gr++) {
#endif
...
#else
                                                                                PTR_INIT( 1 ); BRANCH( 1 ); INDIRECT( 2 ); LOOP( 1 );
      for (gr = 0; gr < aNoIccBins[h_ps_dec->freqResIcc?1:0]; gr++) {
#endif
