/****************************************************************************
 ...
 $Id: ps_dec.c,v 1.1.2.11 2004/04/23 10:29:02 je Exp $

*******************************************************************************/
/*
  \file
  \brief  Parametric stereo decoding $Revision: 1.1.2.11 $
			...
			float* overlapBuf)
{
  ...
  PTR_INIT(7);
  int xxx =sizeof(hHeaderData->sbr_OverlapBuffer);
  ptr1 = &hHeaderData->sbr_OverlapBuffer[MAXNRSBRCHANNELS-1][0];
  ptr2 = &overlapBuf[OverlapBufferSize];
  ptr3 = &hHeaderData->sbr_CodecQmfStatesAnalysis[QMF_FILTER_STATE_ANA_SIZE];
  ptr4 = &hHeaderData->sbr_GainSmooth[MAXNRSBRCHANNELS-1][0];
  ptr5 = &hHeaderData->sbr_NoiseSmooth[MAXNRSBRCHANNELS-1][0];
  ptr6 = &hLppTrans->sbr_LpcFilterStatesReal[MAXNRSBRCHANNELS-1][0][0];
  ptr7 = &hLppTrans->sbr_LpcFilterStatesImag[MAXNRSBRCHANNELS-1][0][0];
  ...
  pErr = (int*)CreateHybridFilterBank_dec ( &h_ps_dec->hHybrid,
...
{
  FLC_sub_start( "ApplyPsSlot" );

  HybridAnalysis_dec ( (const float**)rIntBufferLeft,
  ...
  HybridSynthesis_dec ( (const float**)&h_ps_dec->mHybridRealLeft,
                    (const float**)&h_ps_dec->mHybridImagLeft,
                    rIntBufferLeft,
                    iIntBufferLeft,
                    h_ps_dec->hHybrid );                                       INDIRECT( 3 ); FUNC( 5 );

  HybridSynthesis_dec ( (const float**)&h_ps_dec->mHybridRealRight,
            ...
            )
{
  ...
  for (gr=0; gr < NO_IID_GROUPS; gr++) {
    bin = ( ~NEGATE_IPD_MASK ) & bins2groupMap_dec[gr];                             LOGIC( 1 );
                                                                                ADD( 1 ); BRANCH( 1 );
    if (gr == SUBQMF_GROUPS) {
  ...
  FLC_sub_start( "deCorrelate_transDet" );
  ...
  for (gr=0; gr < NO_IID_GROUPS; gr++) {
    ...
    for (sb = groupBorders[gr]; sb < maxsb; sb++) {
      ...
      if (gr >= SUBQMF_GROUPS && sb >= NO_ALLPASS_CHANNELS) {
        FLC_sub_start( "deCorrelate_delay" );
        ...
        FLC_sub_start( "deCorrelate_allpass" );
                                                                                  ADD( 2 ); LOGIC( 1 ); BRANCH( 1 );
                     ...
                     int usb)
{
  ...
  FLC_sub_start( "InitRotationEnvelope" );
  ...
  if (env == 0)
  {
    ...
    if (usb != pms->lastUsb && pms->lastUsb!=0)
    {
      ...
      /*
        ...
        **aaaRealDelayRBufferSerQmf[NO_SERIAL_ALLPASS_LINKS];
        **aaaImagDelayRBufferSerQmf[NO_SERIAL_ALLPASS_LINKS];
      */
#endif
    }
  }
  ...
  {
    bin = ( ~NEGATE_IPD_MASK ) & bins2groupMap_dec[group];                          LOGIC( 1 );
